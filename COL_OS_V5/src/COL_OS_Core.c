...
void Os_cola_init(cola_t *cola, int size)
{
	uint32_t array[size];
...
uint32_t Os_cola_leer(cola_t *cola, int id, int modo)
{
	uint32_t dato = 0;
	int estado = 0;
	...
	if(modo == unBLOCKED)
	{
		while(estado == 0)
		{
			if(cola->data_index > 0)
			{
				dato = cola->data[cola->data_index - 1];
				cola->data[cola->data_index - 1] = 0;
				cola->data_index--;
				estado = 1;
			}
			else
...
uint32_t Os_cola_escribir(cola_t *cola, uint32_t dato, int id, int modo)
{

	int estado = 0;

	/* Si se desea escribir un dato y la cola esta llena se queda esperando hasta que
	 * una posicion de memoria este disponible.
	 * */
	...
	if(modo == unBLOCKED)
	{
		...
		while(estado == 0)
		{
			...
			if(cola->data_index < cola->data_size)
			{
				cola->data[cola->data_index] = dato ;
				cola->data_index++;
				estado = 1;
			}
			else
	...
	/* Se implementa la opcion de bloquear la tarea por los requeriemientos
	 * No se recomienda utilizar esta opcion porque si se presenta esta situacion se finaliza la tarea.
	 * por que la funcion no pasa ninguna tarea a estado ejecucion.
	 * Las colas son completaemnte independientes de las tareas.
	 * */
...
// Los semaforos son colas de un solo dato

void Os_semaforo_init(cola_t *semaforo)
{
	Os_cola_init(semaforo, 1);
}
uint32_t Os_semaforo_leer(cola_t *semaforo, int id, int modo)
{
	return Os_cola_leer(semaforo, id, modo);
}
uint32_t Os_semaforo_escribir(cola_t *semaforo, int id, int modo)
{
	return Os_cola_escribir(semaforo, 1, id, modo);
}
